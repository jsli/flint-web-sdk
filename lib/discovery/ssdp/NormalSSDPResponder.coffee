...
class NormalSSDPResponder extends EventEmitter
    ...
    start: (interval) ->
        if @started
            throw 'NormalSSDPResponder already started'
    ...
    stop: ->
        if not @started
            console.warn 'NormalSSDPResponder is not started'
            return
...
module.exports = NormalSSDPResponder
